...
const classesExtended = $derived(
	`${expandedPadding} ${expandedGap} ${expandedClasses}`,
);
...
>
  ...
  {#if label && !ctx.expanded}
    <div class="{labelBase} {labelClasses}" data-testid="nav-tile-label">
      {label}
    </div>
  {/if}
  <!-- Label (expanded) -->
  {#if labelExpanded && ctx.expanded}
    <div
      class="{labelExpandedBase} {labelExpandedClasses}"
