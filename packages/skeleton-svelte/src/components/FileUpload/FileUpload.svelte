...
>
  <div {...api.getDropzoneProps()}>
    ...
    {:else}
      ...
      >
        <!-- Icon -->
        {#if iconInterface}<span
            class={interfaceIcon}
            data-testid="uploader-interface-icon"
            >{@render iconInterface()}</span
          >{/if}
        <!-- Label -->
        {#if label}<p
            class={interfaceText}
            data-testid="uploader-interface-label"
          >
            {label}
          </p>{/if}
        <!-- Subtext -->
        {#if subtext}<small
            class={interfaceSubtext}
            data-testid="uploader-interface-subtext">{subtext}</small
          >{/if}
      </div>
    {/if}
  </div>
      ...
      {#each api.acceptedFiles as file}
        ...
        >
          ...
          >
            {#if iconFile}<span
                class={fileIcon}
                data-testid="uploader-file-icon">{@render iconFile()}</span
              >{/if}
            <span>{file.name}</span>
          </p>
          <!-- Size -->
