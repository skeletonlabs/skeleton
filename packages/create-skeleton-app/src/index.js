...
async function main() {
	...
	const defaults = new SkeletonOptions();
...
Problems? Open an issue on ${cyan("https://github.com/skeletonlabs/skeleton/issues")} if none exists already.`);
	...
	if (!("path" in opts)) {
		opts.path = await text({
			message:
				"Where should we install your project (Enter for current directory)?",
			placeholder: "",
			validate(value) {
	...
	if (opts?.skeletontemplate) {
		...
		opts.skeletontemplate = resolve(
			opts.skeletontemplatedir,
	...
	if (!templateFound) {
		const metaFiles = fg.sync(["**/csa-meta.json"], {
			cwd: opts.skeletontemplatedir,
			deep: 2,
		});
	...
	if (opts.meta.requiredFeatures) {
		for (const val of opts.meta.requiredFeatures) {
	...
	if (!("skeletontheme" in opts)) {
		const themeChoices = [
			...
			{ label: "Crimson", value: "crimson" },
			{
				label: cyan("Custom"),
				value: "custom",
				hint: "Will ask for a name next",
			},
		];
		if (opts.meta.type === "premium") {
			themeChoices.unshift({
				label: "Use templates built in theme",
				value: "builtin",
			});
		}
	...
	const packages = [
		{
			value: "forms",
			label: "Add Tailwind forms?",
			package: "@tailwindcss/forms",
			force: false,
		},
		{
			value: "typography",
			label: "Add Tailwind typography?",
	...
	const optionalPackages = packages.filter((pkg) => !pkg.force);
	...
	if (
		!["eslint", "prettier", "playwright", "vitest", "inspector"].every(
			(value) => {
