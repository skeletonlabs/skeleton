...
export function dist(pathToFind) {
	let pathAdjust = "";
	const base = fileURLToPath(new URL(`./`, import.meta.url).href);
...
export function checkIfDirSafeToInstall(path) {
	...
	conflicts = conflicts.filter((file) =>
		/^(package.json|svelte.config.js|tailwind.config.cjs|postcss.config.cjs|vite.config.ts)$/.test(
			file,
		),
	);
	...
	if (conflicts.length > 10) {
		const err = new Error(
			`The directory ${path} contains too many files/folders to safely install.`,
		);
		throw err;
	}
	return true;
...
export function getHelpText() {
	...
	const data = [
		...
		},
		{
			...
			"Quiet Default": "",
			Value: "",
			Description: "Quiet mode - see below",
		},
		{
			Option: "--name",
			Short: "-n",
		...
		},
		{
			...
			"Quiet Default": "true",
			Value: "true|false",
			Description: "Whether Prettier is added",
		},
		{
			Option: "--eslint",
			Short: "",
			"Quiet Default": "true",
			Value: "true|false",
			Description: "Whether ESLint is added",
		},
		{
			Option: "--playwright",
			Short: "",
			"Quiet Default": "false",
			Value: "true|false",
			Description: "Whether Playwright is added",
		},
		{
			Option: "--vitest",
			Short: "",
			"Quiet Default": "false",
			Value: "true|false",
			Description: "Whether Vitest is added",
		},
		{
			Option: "--codeblocks",
			Short: "",
		...
		},
		{
			...
			"Quiet Default": "false",
			Value: "true|false",
			Description: "Install popups dependencies",
		},
		{
			Option: "--mdsvex",
			Short: "",
		...
		},
		{
			...
			"Quiet Default": "false",
			Value: "true|false",
			Description: "Install Tailwinds Forms plugin",
		},
		{
			Option: "--typography",
			Short: "",
	...
	return (
		columnify(data, {
			columns: ["Option", "Short", "Default", "Value", "Description"],
		}) +
		`
