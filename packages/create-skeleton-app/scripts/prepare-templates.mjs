...
async function copyTemplates() {
	...
	metaFiles.forEach(async (metaFile) => {
		...
		if (csaMeta.type == "premium") {
			...
			fs.rmSync(join("templates", metaFile.split("/")[0], "static", "fonts"), {
				recursive: true,
			});
			let appPostcss = fs.readFileSync(
				join("templates", metaFile.split("/")[0], "src", "app.postcss"),
				"utf8",
			);
