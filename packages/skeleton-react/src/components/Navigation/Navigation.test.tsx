...
describe("Navigation", () => {
	describe("<Navigation.Rail>", () => {
		...
		it("Renders the component", () => {
			const { getByTestId } = render(<Navigation.Rail />);
		...
		describe("Header", () => {
			...
			it("Renders the component", () => {
				const { getByTestId } = render(<Navigation.Rail header="header" />);
				expect(getByTestId(testId)).toBeInTheDocument();
			...
			for (const prop of [
				...
				it(`Correctly applies the \`${prop}\` prop`, () => {
					const value = "foo";
					const { getByTestId } = render(
						<Navigation.Rail header="header" {...{ [prop]: value }} />,
					);
					expect(getByTestId(testId)).toHaveClass(value);
				});
			}
		...
		describe("Tiles", () => {
			...
			it("Renders the component", () => {
				const { getByTestId } = render(
					<Navigation.Rail>tiles</Navigation.Rail>,
				);
				expect(getByTestId(testId)).toBeInTheDocument();
			...
			for (const prop of [
				...
				it(`Correctly applies the \`${prop}\` prop`, () => {
					const value = "foo";
					const { getByTestId } = render(
						<Navigation.Rail {...{ [prop]: value }}>tiles</Navigation.Rail>,
					);
					expect(getByTestId(testId)).toHaveClass(value);
				});
			}
		...
		describe("Footer", () => {
			...
			it("Renders the component", () => {
				const { getByTestId } = render(<Navigation.Rail footer="footer" />);
				expect(getByTestId(testId)).toBeInTheDocument();
			...
			for (const prop of [
				...
				it(`Correctly applies the \`${prop}\` prop`, () => {
					const value = "foo";
					const { getByTestId } = render(
						<Navigation.Rail footer="footer" {...{ [prop]: value }} />,
					);
					expect(getByTestId(testId)).toHaveClass(value);
				});
			}
	...
	describe("<Navigation.Bar>", () => {
		...
		it("Renders the component", () => {
			const { getByTestId } = render(<Navigation.Bar />);
			expect(getByTestId(testId)).toBeInTheDocument();
		});
	...
	describe("<Navigation.Tile>", () => {
		...
		it("Correctly applies the `gap` or `expandedGap` prop based on the `expanded` prop", () => {
			...
			const { getByTestId, rerender } = render(
				<Navigation.Rail expanded={false}>
					<Navigation.Tile gap={gap} expandedGap={expandedGap} />
					);
				</Navigation.Rail>,
			);
		...
		it("Correctly applies the `active` prop based on wether the tile is active", async () => {
			...
			const { getByTestId, rerender } = render(
				<Navigation.Rail>
					<Navigation.Tile active={active} selected={false} />
				</Navigation.Rail>,
			);
			expect(getByTestId(testId)).not.toHaveClass("active");
			rerender(
				<Navigation.Rail>
					<Navigation.Tile active={active} selected={true} />
				</Navigation.Rail>,
			);
		...
		describe("Label", () => {
			...
			for (const prop of ["labelBase", "labelClasses"]) {
				it(`Correctly applies the \`${prop}\` prop`, () => {
					...
					const { getByTestId } = render(
						<Navigation.Rail>
							<Navigation.Tile label="label" {...{ [prop]: value }} />
						</Navigation.Rail>,
					);
					expect(getByTestId(testId)).toHaveClass(value);
				});
		...
		describe("LabelExpanded", () => {
			...
			it("Renders the component", () => {
				const { getByTestId } = render(
					<Navigation.Rail expanded={true}>
						<Navigation.Tile labelExpanded="labelExpanded" />
					</Navigation.Rail>,
				);
				expect(getByTestId(testId)).toBeInTheDocument();
