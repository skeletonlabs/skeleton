...
import { Accordion, AccordionRootContext } from "./Accordion";
...
describe("Accordion usage", () => {
	it("should show the panel when the control is clicked", async () => {
		const { queryByText, getByText } = render(
			<Accordion>
				<Accordion.Item>
					<Accordion.Control controls="testControl">Test Control 1</Accordion.Control>
					<Accordion.Panel id="testControl">Test Panel 1</Accordion.Panel>
				</Accordion.Item>
				<Accordion.Item>
					<Accordion.Control controls="testControl2">Test Control 2</Accordion.Control>
...
describe("<Accordion.Item>", () => {
	...
	it("should allow you to set the `classes` style prop", () => {
		const tailwindClasses = "bg-green-500";
		const { getByTestId } = render(<Accordion.Item classes={tailwindClasses} />);
		expect(getByTestId("accordion-item")).toHaveClass(tailwindClasses);
	});
});
...
describe("<Accordion.Control>", () => {
	it("should render the component", () => {
		const { getByTestId } = render(<Accordion.Control controls="testControl" />);
	...
	it("should be set disabled by `disabled` prop", async () => {
		const { getByTestId } = render(<Accordion.Control controls="testControl" disabled />);
		await waitFor(() => {
...
describe("<Accordion.Panel>", () => {
	...
	it("should allow for children", () => {
		const value = "foobar";
		const { getByTestId } = render(
			<AccordionRootContext.Provider value={mockProvidervalues}>
