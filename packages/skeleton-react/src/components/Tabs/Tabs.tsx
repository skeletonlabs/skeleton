...
const TabsRoot: FC<TabsRootProps> = ({
	...
	return (
		<div
			...
			data-testid="tabs"
		>
			<TabsContext.Provider value={{ fluid, api }}>
				{children}
...
const TabsList: FC<TabsListProps> = ({
	...
	return (
		<div
			{...ctx.api.getListProps()}
			className={`${base} ${justify} ${border} ${margin} ${gap} ${classes}`}
...
const TabsControl: FC<TabsControlProps> = ({
	...
	return (
		...
		>
			{/* Label */}
			<div
				className={`${labelBase} ${rxLabelActive} ${labelClasses}`}
				style={commonStyles}
