...
describe("<Tabs.Control>", () => {
	...
	it("should set `aria-controls` to `controls` value", () => {
		...
		const { getByTestId } = render(
			<Tabs.Control name="test" group="test" controls={controls} />
		);
		expect(getByTestId("tabs-control")).toHaveAttribute(
			"aria-controls",
			controls
		);
	});
	...
	it("should set `aria-label` to `label` value", () => {
		...
		const { getByTestId } = render(
			<Tabs.Control name="test" group="test" label={label} />
		);
		expect(getByTestId("tabs-control").parentElement).toHaveAttribute(
			"aria-label",
			label
		);
	});
	...
	it("should allow you to set the `base` style prop", () => {
		...
		const { getByTestId } = render(
			<Tabs.Control name="test" group="test" base={tailwindClasses} />
		);
		expect(getByTestId("tabs-control").parentElement).toHaveClass(
			tailwindClasses
		);
	});
	...
	it("should allow you to set the `classes` style prop", () => {
		const tailwindClasses = "bg-blue-600";
		const { getByTestId } = render(
			<Tabs.Control name="test" group="test" classes={tailwindClasses} />
		);
		expect(getByTestId("tabs-control").parentElement).toHaveClass(
			tailwindClasses
		);
