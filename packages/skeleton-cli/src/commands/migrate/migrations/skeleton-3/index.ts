...
export default async function (options: MigrateOptions) {
	...
	for (const file of [packageJson, appHtml, appCss]) {
		...
		if (file.paths.length > 1) {
			cli.error(
				`Multiple "${file.name}" entries found in directory: "${cwd}", please ensure there is only one`,
			);
		}
	}
	...
	try {
		...
		const skeletonVersion = await getLatestVersion("@skeletonlabs/skeleton", {
			version: ">=3.0.0-0 <4.0.0",
		});
		const skeletonSvelteVersion = await getLatestVersion(
			"@skeletonlabs/skeleton-svelte",
			{ version: ">=1.0.0-0 <2.0.0" },
		);
		const transformedPackageJson = transformPackageJson(
			packageJsonCode,
			skeletonVersion,
			skeletonSvelteVersion,
		);
		isUsingComponents = transformedPackageJson.meta.isUsingComponents;
		migrations.push({
			path: packageJson.paths[0],
			content: transformedPackageJson.code,
		});
		packageSpinner.stop(`Successfully migrated ${packageJson.name}!`);
	} catch (e) {
		packageSpinner.stop(
			`Failed to migrate ${packageJson.name}: ${e instanceof Error ? e.message : "Unknown error"}`,
			1,
		);
		cli.error("Migration canceled, nothing written to disk");
	}
	...
	try {
		...
		if (
			transformedAppHtml.meta.theme &&
			Object.hasOwn(THEME_MAPPINGS, transformedAppHtml.meta.theme.value)
		) {
		...
		migrations.push({
			path: appHtml.paths[0],
			content: transformedAppHtml.code,
		});
		appHtmlSpinner.stop(`Successfully migrated ${appHtml.name}!`);
	} catch (e) {
		appHtmlSpinner.stop(
			`Failed to migrate ${appHtml.name}: ${e instanceof Error ? e.message : "Unknown error"}`,
			1,
		);
		cli.error("Migration canceled, nothing written to disk");
	}
	...
	try {
		...
		const transformedAppCss = transformAppCss(
			appCssCode,
			theme ?? FALLBACK_THEME,
			isUsingComponents,
		);
		...
		appCssSpinner.stop(
			`Failed to migrate ${appCss.name}: ${e instanceof Error ? e.message : "Unknown error"}`,
			1,
		);
		cli.error("Migration canceled, nothing written to disk");
	}
	...
	if (sourceFolders.length > 0) {
		...
		for (const sourceFile of sourceFiles) {
			...
			try {
				...
				sourceFilesSpinner.stop(
					`Failed to migrate ${sourceFile}: ${e instanceof Error ? e.message : "Unknown error"}`,
					1,
				);
				cli.error("Migration canceled, nothing written to disk");
			}
	...
	try {
		await Promise.all(
			migrations.map(({ path, content }) => writeFile(path, content)),
		);
	...
	try {
		...
		installDependenciesSpinner.stop(
			`Failed to update dependencies: ${e instanceof Error ? e.message : "Unknown error"}`,
			1,
		);
		cli.error("Migration canceled");
		return;
	...
	log.info(
		"Migration complete! Visit https://skeleton.dev for more information.",
	);
}
