<script lang="ts">
	...
	// Check if ID was passed. If not, generate a random one.
	onMount(() => {
		id !== '' ? (idPassed = true) : (id = String(Math.random()));
	});
	...
	function onToggle(event?: SvelteEvent<MouseEvent, HTMLButtonElement>): void {
		...
		dispatch('toggle', { event, id: idPassed ? id : `{accordion-control-${id}`, open: currentOpenState, autocollapse });
	}
...
<div class="accordion-item {classesBase}" data-testid="accordion-item">
	...
	<button
		...
		id={idPassed ? id : `accordion-control-${id}`}
		on:click={setActive}
		on:click
		on:keydown
		on:keyup
	...
	{#if openState}
		<div
			...
			aria-labelledby={idPassed ? id : `accordion-control-${id}`}
		>
