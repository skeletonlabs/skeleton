<script lang="ts">
	...
	switch (position) {
		...
		// Corners
		case ('tl'): cPosition = 'justify-start items-start'; cAlign = 'items-start'; animAxis = { x: -100, y: -100 }; break;
		case ('tr'): cPosition = 'justify-end items-start'; cAlign = 'items-end'; animAxis = { x: 100, y: -100 }; break;
		case ('bl'): cPosition = 'justify-start items-end'; cAlign = 'items-start'; animAxis = { x: -100, y: 100 }; break;
		case ('br'): cPosition = 'justify-end items-end'; cAlign = 'items-end'; animAxis = { x: 100, y: 100 }; break;
	}
...
{#if $toastStore.length}
	<!-- Wrapper -->
	<div class="snackbar-wrapper {classesWrapper} w-">
		<!-- List -->
		<div class="snackbar {classesSnackbar}" transition:fly={{ x: animAxis.x, y: animAxis.y, duration }}>
			{#each $toastStore as t, i}
				{#if i < max + 1}
					<!-- Toast -->
					<div class="toast {classesBase}">
						<div class="text-base">{t.message}</div>
						<!-- prettier-ignore -->
						<div class="flex items-center space-x-2">
							{#if t.button}<button class="btn btn-ghost" on:click={onAction}>{t.button.label}</button>{/if}
							<button class="btn-icon btn-filled" on:click={() => { toastStore.close(i) }}>✕</button>
						</div>
					</div>
				{/if}
			{/each}
