...
<script lang="ts">
	...
	let svg: any = {
		...
		icon: '<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24"><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9c.83 0 1.5-.67 1.5-1.5 0-.39-.15-.74-.39-1.01-.23-.26-.38-.61-.38-.99 0-.83.67-1.5 1.5-1.5H16c2.76 0 5-2.24 5-5 0-4.42-4.03-8-9-8zm-5.5 9c-.83 0-1.5-.67-1.5-1.5S5.67 9 6.5 9 8 9.67 8 10.5 7.33 12 6.5 12zm3-4C8.67 8 8 7.33 8 6.5S8.67 5 9.5 5s1.5.67 1.5 1.5S10.33 8 9.5 8zm5 0c-.83 0-1.5-.67-1.5-1.5S13.67 5 14.5 5s1.5.67 1.5 1.5S15.33 8 14.5 8zm3 4c-.83 0-1.5-.67-1.5-1.5S16.67 9 17.5 9s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"></path></svg>'
	}
...
<Menu {select} {open} {origin} {duration} {disabled} class="themeswitcher {$$props.class}" data-testid="themeswitcher">
	...
	<Card slot="content" class="bg-surface-300 dark:bg-surface-700 p-0 overflow-hidden">
		<List role="nav" class="fill-surface-500 mb-1">
			<div class="flex items-center justify-between mx-2 mt-3 align-middle">
				<span>{@html svg.light}</span>
...
<style lang="postcss">
    .theme-set {
        @apply m-2 text-white text-center p-2 my-1 justify-between items-center border border-white/10 hover:border-white/50 rounded shadow cursor-pointer;
    }
</style>