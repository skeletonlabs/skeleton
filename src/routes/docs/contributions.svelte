...
<div class="space-y-8">
    ...
    <section class="space-y-4">
        ...
        <ul class="list-disc list-inside">
            <li>Pull requests are always welcome. Read this full guide, <a href="https://github.com/Brain-Bones/skeleton/issues" target="_blank">create or pick an issue</a>, stake your claim, and start helping!</li>
            <li>Let us know what you want to see added. We are actively seeking feedback on the <a href="https://github.com/Brain-Bones/skeleton/wiki/%F0%9F%9B%A3%EF%B8%8F-The-Skeleton-Roadmap-(proposal)" target="_blank">roadmap</a>, see the <a href="https://github.com/Brain-Bones/skeleton/discussions/79" target="_blank">discussion thread</a>.</li>
    ...
    <section class="space-y-4">
        ...
        <ol class="list-decimal list-inside">
            <li><a href="https://github.com/Brain-Bones/skeleton/issues" target="_blank">Select an issue</a> or <a href="https://github.com/Brain-Bones/skeleton/wiki/%F0%9F%9B%A3%EF%B8%8F-The-Skeleton-Roadmap-(proposal)" target="_blank">roadmap task</a> you wish to make a contribution to.</li>
            <li>Leave a comment, join the <a href="https://github.com/Brain-Bones/skeleton/discussions/79" target="_blank">roadmap discussion</a>, or notify core contributors on Discord.</li>
        ...
        <p>The overwhelming majority of rejected pull requests come from unrequested submissions that do not meet the requirements for a feature or otherwise conflict with other ongoing efforts. To avoid rejection, be a team player, don't try and be a hero!</p>
    </section>
    ...
    <section class="space-y-4">
        <h2>Pull Requests</h2>
        <p>As a common courtesy, please reach out to contributors before you start work on new features. If you're interested in creating a new feature, create a ticket on <a href="https://github.com/Brain-Bones/skeleton/issues" target="_blank">Github Issues</a>. This allows for open discourse and may help prevent redudant work. This includes but is not limited to: creating new components, adding new utilities, or alterations to existing work.</p>
        <h4>Git Branches</h4>
        <DataTable headings="{tableBranches.headings}" source="{tableBranches.source}"></DataTable>
        ...
        <DataTable headings="{tableBranchConventions.headings}" source="{tableBranchConventions.source}"></DataTable>
    </section>
    ...
    <section class="space-y-4">
        <h2>Dependencies</h2>
        <p>Skeleton aims to keep depedencies to as few as possible to keep the project lean. Please be leery of introducing new dependencies. However, don't be afraid to discuss additions you would like to propose with core contributors before proceeding. We just want what's best for end users!</p>
    </section>
    ...
    <section class="space-y-4">
        ...
        <DataTable headings="{tableProjectStructure.headings}" source="{tableProjectStructure.source}"></DataTable>
        <h4>Component Naming Conventions</h4>
        <ul class="list-disc list-inside">
            <li>Components and their wrapping directory should be singular and title-case (ex: <code>/lib/LightSwitch/LightSwitch.svelte</code>)</li>
            ...
            <li>Documentation pages represent a URL and should be plural, all lowercase, and separated by dashes (ex: <code>/routes/components/radio-groups.svelte</code>)</li>
            <li>Documentation anchor labels should be capitalized and and plural form (ex: <code>Radio Groups</code>)</li>
        </ul>
    </section>
...
<div class="bg-accent-500">Skeleton</div> ✅ 
        `.trim()}></CodeBlock>
        <p>If you need to include miscellaneous attributes that were not defined as props, use Svelte's <code>$$restProps</code>. Be careful though, this can overwrite the element's <code>$$props.class</code> attribute. To avoid this, delete the <code>class</code> key from <code>$$restProps</code>. The function provided below can handle this on both init and after any form of attribute updates.</p>
        <CodeBlock language="js" code={`
function prunedRestProps(): any {
    delete $$restProps.class;
    return $$restProps;
}`.trim()}></CodeBlock>
        <CodeBlock language="html" code={`<button class="... {$$props.class}" {...prunedRestProps()}>Skeleton</button>`}></CodeBlock>
    </section>
...
$: classesLabel = \`\${cBaseLabel}\`; // child element
        `.trim()}></CodeBlock>
        <p>Apply the reactive classes as follows. Make sure to also include <code>$$props.classes</code> as a pass-through for additional classes. Note that the first class acts as an identifier for the element, which is useful for testing and DOM manipulation.</p>
        <CodeBlock language="html" code={`
<div class="card {classesCard} {$$props.classes||''}">
    ...
    <section class="space-y-4">
        ...
        <ul class="list-disc list-inside">
            <li>As a best practice for Tailwind, avoid component <code>style</code> blocks. This can increase the final stylesheet bundle size.</li>
    ...
    <section class="space-y-4">
        ...
        <p>A boilerplate template for documentation pages is available under <code>/routes/components/_template.svelte</code>.</p>
        <h4>Update Site Navigation</h4>
        <p>Open <code>/routes/__layout.svelte</code> and modify the contents of <code>navigation</code> definition to link to any documentation pages you add.</p>
        <h4>Interactive vs Static Demos</h4>
        <p>While interactive demos are preferred, static demos are fine. Just make sure to provide a variety of examples to illustrate potential use cases and showcase the feature set of the component. If you need help adding an interactive demo, please contact any core contributor on the team.</p>
    </section>
    ...
    <section class="space-y-4">
        <h2>Running Tests</h2>
        <p>Tests are handled via <a href="https://vitest.dev/" target="_blank">Vitest</a>, which uses similar conventions and syntax to Jest. Please ensure you write and run tests before submitting a pull request. Bare minimum, these tests should test components with the minimal and maximum available prop changes.</p>
