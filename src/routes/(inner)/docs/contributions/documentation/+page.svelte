<script lang="ts">
	import { Alert, Breadcrumb, Crumb, CodeBlock } from '@brainandbones/skeleton';
</script>
...
<div class="page-container">
	...
	<header class="space-y-4">
		<div class="grid grid-cols-[1fr_auto] gap-4">
			<div class="space-y-4">
				<h1>Documentation</h1>
				<p>
					Below is an introduction to automated documentation features, using tools like <a href="https://github.com/carbon-design-system/sveld" target="_blank" rel="noreferrer">Sveld</a>. First we'll
					learn how to document component props, slots, and events. Then we'll cover how to provide this information to the documentation pages themselves.
				</p>
			</div>
			<div class="place-self-end grid grid-cols-1 gap-2">
				<a class="btn btn-filled-primary" href="https://github.com/carbon-design-system/sveld" target="_blank" rel="noreferrer">Sveld Documentation</a>
			</div>
		</div>
	</header>
	...
	<section class="space-y-4">
		...
		<p>
			To document component props, simply add TSDoc description comments using the <code>/** ... */</code> format. In most use cases Sveld will automatically parse all relevant information - including
			the prop name, type, value, and your provided description.
		</p>
		<CodeBlock
			language="js"
			code={`
...
export let space: string = 'space-y-1';
		...
		/>
		<p>
			For non-primitive or custom types, you may need to specify this information. This can be accomplished using the <code>@type</code> tag with block-style comments. Specify the type in curly brackets
 ...
 */`}
		/>
		<Alert><strong>NOTE:</strong> The leading <code>// ...</code> comment is required for Sveld to successfully parse the slot descriptions. This is not optional.</Alert>
	</section>
	...
	<section class="space-y-4">
		...
		<p>
			Sveld will automatically document forwarded events. You should not attempt to document these. However, dispatched events may be documented similar to props - with a TSDocs comment applied
			directly above the <code>dispatch()</code> method. Provide the event response in curly brackets, followed by the event name, a dash, and then the event description.
		</p>
		<CodeBlock
			language="js"
			code={`
...
dispatch('dragover', event);`}
	...
	<section class="space-y-4">
		...
		<p>Now that our components are ready, it's time to create the documentation page that displays all of the information derived by Sveld.</p>
		<p>
		...
		<p>
			We can provide settings to our DocShell component using <code>const settings: DocsShellSettings</code>. This allows you to populate all relevant settings on the page.
		</p>
		...
		<h3>Usage</h3>
		<p>
			In addition to examples, you should provide multiple use case demonstrations using <code>CodeBlock</code> snippets to help end developers understand how to make use of your new components and features.
		</p>
		<!-- Keyboard Interactions -->
		<h3>Keyboard Interactions</h3>
		<p>Finally, don't forget to document any relevant keyboard interactions for accessability. These can easily be provided via the DocShell settings.</p>
	</section>
</div>
