			...
			<div class="card card-body p-4">
				<Stepper {active} length={6} on:complete={onComplete}>
					...
					<Step index={2} locked={!exampleLockedState}>
						...
						<SlideToggle name="unlock" bind:checked={exampleLockedState}>Unlock</SlideToggle>
					</Step>
					<Step index={3}>
						<svelte:fragment slot="header"><h4>Step 4 - <code>clickNavigation</code> Property</h4></svelte:fragment>
						<p>
							Steps can also be made clickable by setting the <code>clickNavigation</code> prop in Stepper to <code>true</code>. This makes
							all steps clickable that have already been reached.
						</p>
					</Step>
					<Step index={4}>
