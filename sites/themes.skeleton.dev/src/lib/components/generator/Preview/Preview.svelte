...
function copyToClipboard() {
	if (!window.isSecureContext) {
		console.error(
			"Needs secure context: https://developer.mozilla.org/en-US/docs/Web/API/Clipboard",
		);
		return {};
	}
...
<section
  class="relative h-screen overflow-y-auto bg-body-color dark:bg-body-color-dark"
>
  <!-- Content -->
    ...
    {:else}
      ...
      <section class="p-10 space-y-5">
        <header class="flex justify-between gap-4">
          ...
          <button
            type="button"
            class="btn preset-outlined-surface-300-700 hover:preset-tonal"
  ...
  <footer class="p-10 py-5">
    <p class="text-xs text-center opacity-20">
