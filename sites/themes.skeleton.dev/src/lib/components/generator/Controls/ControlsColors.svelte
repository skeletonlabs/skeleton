...
import {
	...
	genRandomSeed,
	seedColor,
} from "$lib/utils/generator/colors";
// Components (Skeleton)
...
// State
const showAllShades = $state(false);
...
function promptColorSeed(colorName: string) {
	const promptSeed = prompt(
		`Automatically generate a ${colorName} color palette from a hex color value that you provide.`,
	);
  ...
  >
    {#snippet list()}
      {#each colorSelection as color}
        <Tabs.Control
          value={color.value}
          labelBase="flex justify-center"
          stateInactive=""
        >
          <div
            class="aspect-square w-[52px] rounded-md flex justify-center items-center {color.class}"
    ...
    {#snippet content()}
      {#each colorSelection as color}
        {#if color.value === globals.activeColor}
          {@const activeColorLabel = colorSelection.find(
            (c) => c.value === globals.activeColor,
          )?.label}
          <div class="space-y-4">
            <!-- Actions -->
            <div class="grid grid-cols-[1fr_auto_auto_auto] items-center gap-2">
              ...
              <Switch
                name="example"
                checked={showAllShades}
                onCheckedChange={(e) => (showAllShades = e.checked)}
                classes="!gap-2"
              >
            ...
            <div>
              ...
              {:else}
                <p class="opacity-60">
                  Shades automatically blend between 50/500/950.
                </p>
              {/if}
            </div>
            <!-- Table: Shades -->
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        class="input"
                        bind:value={
                          settingsColors[`--color-${color.value}-${shade}`]
                        }
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <label class="label space-y-2">
                  ...
                  <select
                    ...
                    bind:value={
                      settingsColors[`--color-${color.value}-contrast-light`]
                    }
                  >
                ...
                <label class="label space-y-2">
                  ...
                  <select
                    ...
                    bind:value={
                      settingsColors[`--color-${color.value}-contrast-dark`]
                    }
                  >
