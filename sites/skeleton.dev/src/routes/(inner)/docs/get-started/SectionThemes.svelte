	...
	<div class="space-y-4">
		<nav class="grid grid-cols-1 gap-4 md:grid-cols-2">
			{#each themes.slice(1, themes.length) as preset}
				<div
					class="card card-hover hover:ring-surface-500/50 cursor-pointer space-y-4 p-4 text-white"
					...
					<div class="flex items-center justify-between gap-4">
						...
						<ul class="flex items-center justify-center -space-x-1">
							{#each preset.colors as color}
								<li class="aspect-square w-3 rounded-full xl:w-4" style:background={color} />
							{/each}
						</ul>
					</div>
				</div>
