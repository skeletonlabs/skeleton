      ...
      <svelte:fragment slot="preview">
        <div class="flex snap-x snap-mandatory scroll-px-4 gap-4 overflow-x-auto scroll-smooth px-4 py-10">
          {#each { length: 8 } as _, i}
            <div class="{currentSnap} card w-40 shrink-0 py-20 text-center md:w-80">{i + 1}</div>
          {/each}
        </div>
      </svelte:fragment>
      ...
      <DocsPreview background="neutral" regionFooter="text-center">
        <svelte:fragment slot="preview">
          <div class="grid grid-cols-[auto_1fr_auto] items-center gap-4">
            ...
            <!-- Carousel -->
            <div bind:this={elemMovies} class="flex snap-x snap-mandatory gap-2 overflow-x-auto scroll-smooth pb-2">
