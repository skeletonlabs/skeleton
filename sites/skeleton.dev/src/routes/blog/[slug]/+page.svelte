...
<div class="mx-auto max-w-5xl space-y-8 p-4 md:p-12">
  ...
  <header class="space-y-4">
    ...
    <div class="blog-meta flex items-center justify-between">
      ...
      <span class="hidden text-xs opacity-50 md:block">{post.reading_time} min read</span>
    </div>
    ...
    {#if post.feature_image}<img
        ...
        alt={post.title}
        class="rounded-container-token aspect-video w-full shadow-xl"
      />{/if}
  </header>
  <!-- Article -->
  ...
  <footer class="card variant-glass-surface mb-28 flex items-center justify-between p-4">
    ...
    <div class="flex items-center space-x-4">
      {#each post.tags as tag}
        <span class="text-sm font-bold capitalize opacity-50">{tag.slug}</span>
      {/each}
    </div>
    <!-- Scroll to Top -->
    <!-- prettier-ignore -->
