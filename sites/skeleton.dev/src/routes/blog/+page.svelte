...
<div class="page-container-wide page-padding">
	...
	<section class="blog-list space-y-8">
		{#each data.posts as post}
			<a
				class="hover:card hover:variant-soft rounded-container-token block p-4"
				href="/blog/{post.slug}"
			...
			>
				<article class="grid grid-cols-1 gap-4 lg:grid-cols-[auto_1fr] lg:gap-8">
					...
					{#if post.feature_image}
						<img
							class="rounded-container-token aspect-video w-full bg-black/50 bg-cover bg-center shadow-xl lg:max-w-sm"
					...
					<div class="space-y-4">
						...
						<div class="flex items-center space-x-4">
							{#each post.tags as tag}
								<span class="text-xs font-bold capitalize opacity-50">{tag.slug}</span>
							{/each}
						</div>
