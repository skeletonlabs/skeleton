...
It is highly recommend you read through the entire migration guide to verify all changes are to your preference. And of course ensure you (or your LLM) take into account the remaining items not handled by the CLI migration. Please review the reference below.

| CLI Migrations   | Manual Migrations                                                                                      |
| ---------------- | ------------------------------------------------------------------------------------------------------ |
| Package versions | Svelte peer dependency                                                                                 |
| Design tokens    | Custom theme migration                                                                                 |
| Utility classes  | Structural class changes (ex: [Field Group](/docs/[framework]/tailwind-components/forms#field-groups)) |

---

## Update NPM Packages

Use your package manager of choice to update the core and component packages Skeleton.
...
The Input Group feature has now been replaced with [Field Group](/docs/[framework]/tailwind-components/forms#field-groups). This is functionally similar, but much more intuative to implement and use. This also encourages the use of a semantic HTML `<label>` element.
...
When wrapped in a button, badge, chip, or disclosure the icon size is now handled automatically. If you have manually specified a size in the past, consider removing this to allow the inherit sizing to take effect.
