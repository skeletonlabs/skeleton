...
In this guide we'll implement the Melt UI `<Accordion>` component. This will showcase the bare minimum requirements for integrating Skeleton with Melt UI. You are then responsible for all further components.
...
<Process>
    ...
    <ProcessStep step="3">
    	...
    	Create a new component in `/lib/components/Accordion/Accordion.svelte` and insert the following markup. This will generate an unstyled version o the component.
...
Below is an example showing the full component with fake data and basic configuration.
