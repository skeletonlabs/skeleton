...
import ImgBitsBanner from '@images/headless/bits-ui/bits-header.png';
import ImgBitsCalendar from '@images/headless/bits-ui/bits-calendar.png';

import Process from '@components/docs/Process.astro';
import ProcessStep from '@components/docs/ProcessStep.astro';

<Image src={ImgBitsBanner} alt="banner" class="rounded-container shadow-xl" />
...
In this guide we'll implement the Bits UI `<Calendar>` component. This will showcase the bare minimum requirements for integrating Skeleton with Bits UI. You are then responsible for all further components.

<Image src={ImgBitsCalendar} alt="calendar" class="rounded-container shadow-xl" />
...
<Process>
    <ProcessStep step="1">
		...
		To begin, we'll setup a new SvelteKit project with Skeleton v3 and Tailwind v4.
    ...
    <ProcessStep step="2">
    	...
    	We'll also include the [Adobe International Date](https://react-spectrum.adobe.com/internationalized/date/index.html) package to aid date/time formatting.
    ...
    <ProcessStep step="3">
    	...
    	Create a new component in `/lib/components/Calendar/Calendar.svelte` and insert the following markup. This will generate an unstyled version o the component.
...
For the sake of time, we won't cover the entire component. Instead, here's the fully styled component utilizing Skeleton primitives with some recommend configuration.
...
- Styled examples are provided near the top each component page. These use Tailwind utilty classes.
- Use these styled examples as your foundation, then replace with Skeleton utility classes as needed.
- For the Calendar, consider abstracting the `value` using a [$bindable](https://svelte.dev/docs/svelte/$bindable) prop.
...
- If you wish to match other Skeleton conventions, see our [contributor component guidelines](/docs/resources/contribute/components).
