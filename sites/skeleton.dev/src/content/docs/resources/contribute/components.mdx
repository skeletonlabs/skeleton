...
│   ├── avatar-root-context.{tsx|svelte}
...
The `{component}-{part}-context.ts` file contains the exported context for each part's context. This pattern enables strong typing.
...
> NOTE: The interface is suffixed `Type` to avoid conflicting with the context variable name.
...
It's worth noting that during build time, Skeleton will automatically prefix each class in the class list with `skb:` (this is why we're using `with { type: 'macro' }`, this signals to the build process to run `defineSkeletonClasses` at build time). The purpose of this prefix is to assign each class to the Tailwind `@base` layer, ensuring user-provided classes take precedence over these classes. This is accomplished using the following Tailwind custom variant.
...
If you need to prevent a class from being prefixed at build time you may mark that class explicitely by prefixing the class with `not-skb:`.
