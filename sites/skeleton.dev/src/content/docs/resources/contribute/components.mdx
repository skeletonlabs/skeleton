...
To obtain component source code, locate each respective component on the Zag website.
...
In most cases, Zag presents all source code in a single file. Take care when splitting this into multiple component parts. Most source should be defined in the component root - including the primitive imports, and defining the `machine` and `api`.
...
In some cases you may need to pass data from the parent root component, down to the children. For this, we can utilize each framework's Context API:
...
While each component will present a unique set of requirements, we recommend you adhere to the following common conventions.

- React-Only: Replace the hardcoded `id` with `useId()`, to generate a unique ID per component
- Svelte-Only: Replace the hardcoded `id` with `$props.id()`, to inheret a use-provided ID.
- Avoid hardcoded text or icons. Implement a means for users to override these (props, snippets, sub-components).
- (another)...
- (another)...
