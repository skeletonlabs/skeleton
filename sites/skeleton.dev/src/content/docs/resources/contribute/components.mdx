...
- `HTMLAttributes<Tag>` - from Skeleton's `@/internal/html-attributes`; allow for standard HTML attributes.

#### Omit

- `Omit<Props, 'id'>` - omit the `id` field from the `Props` interface as they will be provided inside the component itself.
- React: `Omit<HTMLAttributes<'div'>, 'id' | 'dir'>` - omit conflicting HTML attributes.
- Svelte: `Omit<HTMLAttributes<'div'>, 'id' | 'dir'>` - omit conflicting HTML attributes.
