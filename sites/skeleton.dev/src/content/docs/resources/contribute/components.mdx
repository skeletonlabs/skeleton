...
- `PropsWithElement<Tag>` - via Skeleton's `@/internal/props-with-element.js`; allows for HTML template overrides.
- `HTMLAttributes<Tag, Omit>` - via Skeleton's `@/internal/html-attributes.js`; allows for standard HTML attributes.
...
```ts
export { Avatar } from './modules/anatomy.js';
