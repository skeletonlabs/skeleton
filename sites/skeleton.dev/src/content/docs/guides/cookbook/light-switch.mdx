...
title: Lightswitch
description: Learn how to create a ModeSwitch toggle.
showDocsUrl: true
pubDate: 2025-03-18
tags: ['theme', 'mode', 'images']
---
...
This guide will show you how to create a ModeSwitch using Tailwind and Skeleton.
...
Here is the implementation for the `ModeSwitch` component:

```svelte
<script lang="ts">
	Import the Switch component, or any other switch component you want to use.
	import { Switch, SwitchProps } from '@skeleton/skeleton-svelte';

	// Define a function to retrieve the initial value of the switch.
	const getInitialValue = () => {
		// We don't have access to localStorage on the server, so we default to light mode.
	...
	const onValueChange: SwitchProps['onValueChange'] = (event) => {
...
<Switch value={getInitialValue()} {onValueChange}></Switch>
```

## Step 3: Add the ModeSwitch to your app

Now that we have the `ModeSwitch` component, we can add it to our app. We recommend adding it to the header of your website, so it's easily accessible to the user.
