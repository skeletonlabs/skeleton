...
import { Page as ExampleInitials } from '@examples/components/avatars/ExampleInitials.tsx';

import ExampleFallbackRaw from '@examples/components/avatars/ExampleFallback.tsx?raw';
import ExampleInitialsRaw from '@examples/components/avatars/ExampleInitials.tsx?raw';
...
<Preview client:load>
	...
	<Fragment slot="code">
		<Code code={ExampleInitialsRaw} lang="tsx" />
	</Fragment>
</Preview>

## Fallback

When `src` or `srcSet` are missing or fail to load, a fallback will be displayed using the default `children`. This can be used to display icons or even pass a [Next.js image component](https://nextjs.org/docs/pages/api-reference/components/image).
