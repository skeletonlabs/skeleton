---
layout: '@/components/layouts/LayoutDoc.astro'
---

import DefaultRaw from '@/components/examples/components/avatar/react/default.tsx?raw';
import Default from '@/lib/components/examples/components/avatar/react/default';

import FallbackRaw from '@/components/examples/components/avatar/react/fallback.tsx?raw';
import Fallback from '@/lib/components/examples/components/avatar/react/fallback';

import FilterRaw from '@/components/examples/components/avatar/react/filter.tsx?raw';
import Filter from '@/lib/components/examples/components/avatar/react/filter';

<Preview client:load>
	<Fragment slot="preview">
		<Default client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={DefaultRaw} lang="tsx" />
	</Fragment>
</Preview>

## Fallback

Use `<Avatar.Fallback>` to provide initials, icons, or a framework-specific image component.

<Preview client:load>
	<Fragment slot="preview">
		<Fallback client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={FallbackRaw} lang="tsx" />
	</Fragment>
</Preview>

## Filter

Avatars can implement [SVG Filters](/docs/guides/cookbook/svg-filters) using the image `className` attribute.

<Preview client:load>
	<Fragment slot="preview">
		<Filter client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={FilterRaw} lang="tsx" />
	</Fragment>
</Preview>

## API Reference

<ApiTable />
