---
layout: '@/components/layouts/LayoutDoc.astro'
---

import DefaultRaw from '@/components/examples/components/pagination/react/default?raw';
import Default from '@/lib/components/examples/components/pagination/react/default';

import DynamicPageSizeRaw from '@/components/examples/components/pagination/react/dynamic-page-size?raw';
import DynamicPageSize from '@/lib/components/examples/components/pagination/react/dynamic-page-size';

import DirRaw from '@/components/examples/components/pagination/react/dir?raw';
import Dir from '@/lib/components/examples/components/pagination/react/dir';

<Preview client:load>
	<Fragment slot="preview">
		<Default client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={DefaultRaw} lang="tsx" />
	</Fragment>
</Preview>

## Page Size

<Preview client:load>
	<Fragment slot="preview">
		<DynamicPageSize client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={DynamicPageSizeRaw} lang="tsx" />
	</Fragment>
</Preview>

## Direction

<Preview client:load>
	<Fragment slot="preview">
		<Dir client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={DirRaw} lang="tsx" />
	</Fragment>
</Preview>

## Total Count

For server-side pagination, your data source may be truncated. Make sure to specify the total records using `count`.

```json
{
	"data": [...],
	"pagination": {
		"page": 1,
		"limit": 10,
		"count": 500,
	}
}
```

{/* prettier-ignore */}
```tsx
<Pagination
	page={response.pagination.page}
	count={response.pagination.count}
	pageSize={response.pagination.limit}
>
	...
</Pagination>
```

## API Reference

<ApiTable />
