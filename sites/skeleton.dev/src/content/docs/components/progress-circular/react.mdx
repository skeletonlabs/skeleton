---
layout: '@/components/layouts/LayoutDoc.astro'
---

import DefaultRaw from '@/components/examples/components/progress-circular/react/default?raw';
import Default from '@/lib/components/examples/components/progress-circular/react/default';

import SizeRaw from '@/components/examples/components/progress-circular/react/size?raw';
import Size from '@/lib/components/examples/components/progress-circular/react/size';

import ColorRaw from '@/components/examples/components/progress-circular/react/color?raw';
import Color from '@/lib/components/examples/components/progress-circular/react/color';

import CenteredContentRaw from '@/components/examples/components/progress-circular/react/centered-content?raw';
import CenteredContent from '@/lib/components/examples/components/progress-circular/react/centered-content';

import IndeterminateRaw from '@/components/examples/components/progress-circular/react/indeterminate?raw';
import Indeterminate from '@/lib/components/examples/components/progress-circular/react/indeterminate';

import FormatRaw from '@/components/examples/components/progress-circular/react/format?raw';
import Format from '@/lib/components/examples/components/progress-circular/react/format';

import CustomValueTextRaw from '@/components/examples/components/progress-circular/react/custom-value-text?raw';
import CustomValueText from '@/lib/components/examples/components/progress-circular/react/custom-value-text';

<Preview client:load>
	<Fragment slot="preview">
		<Default client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={DefaultRaw} lang="tsx" />
	</Fragment>
</Preview>

## Size

<Preview client:load>
	<Fragment slot="preview">
		<Size client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={SizeRaw} lang="tsx" />
	</Fragment>
</Preview>

## Color

<Preview client:load>
	<Fragment slot="preview">
		<Color client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={ColorRaw} lang="tsx" />
	</Fragment>
</Preview>

## Centered Content

<Preview client:load>
	<Fragment slot="preview">
		<CenteredContent client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={CenteredContentRaw} lang="tsx" />
	</Fragment>
</Preview>

## Indeterminate

Set the value to `null` to make the progress indeterminate.

<Preview client:load>
	<Fragment slot="preview">
		<Indeterminate client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={IndeterminateRaw} lang="tsx" />
	</Fragment>
</Preview>

## Format

Use the `format` prop to customize the output of the `ValueText` component, options are:

- `percentage` (default) - shows the percentage value
- `decimal` - shows the decimal value (0.0 - 1.0)
- Provide formatting using the [Intl API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl)

<Preview client:load>
	<Fragment slot="preview">
		<Format client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={FormatRaw} lang="tsx" />
	</Fragment>
</Preview>

## Custom Value Text

<Preview client:load>
	<Fragment slot="preview">
		<CustomValueText client:visible />
	</Fragment>
	<Fragment slot="code">
		<Code code={CustomValueTextRaw} lang="tsx" />
	</Fragment>
</Preview>

## API Reference

<ApiTable component="progress" />
