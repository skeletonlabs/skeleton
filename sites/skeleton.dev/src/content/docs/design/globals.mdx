...
:root {
	...
	/* Defines a global text scaling multiplier. */
	--text-scaling: 1;
	/* Defines the corner shape for base elements (e.g. squircle). */
	--corner-shape-base: inherit;
	/* Defines the corner shape for container elements (e.g. squircle). */
...
```css
html {
	background-color: var(--color-root-bg-light);
	@variant dark {
		background-color: var(--color-root-bg-dark);
	}
}
```
