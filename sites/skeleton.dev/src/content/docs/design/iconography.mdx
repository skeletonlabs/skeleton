...
Icon sizes are based on Tailwind's built in `--text-*` property, which means you can match icon sizes using the following syntax for `size-*`, `w-*` or `h-*`.

```html
<SomeIcon class="icon-{size}" />
