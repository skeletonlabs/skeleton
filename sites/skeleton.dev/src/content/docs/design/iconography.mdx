...
Tailwind Components (buttons, badges, chips, etc) implement sizes automatically for any SVG-based icons (such as Lucide). Meaning explicitly setting the size is optional. The rule of thumb: only apply icon sizes when required.

### Custom Sizes

Sizes are based on Tailwind built-in `@theme` property for `--text-*`. Either continue the sequence or define a custom value.
