...
> Make sure to replace `{theme-name}` with your desired theme names. All lower-case, such as `cerberus`.
...
Then proceed below to learn how to register and activate each theme.

## Register Themes

You may register any number of themes by adding addition theme imports to your global stylesheet. Please note that each theme will slightly increase the final CSS bundle size, so avoid including unused themes.
...
You can modify any preset theme's [CSS custom properties](/docs/[framework]/get-started/core-api) on demand using the following technique. Simply add this to your global stylesheet. For custom themes, modify the properties within the theme file directly.
