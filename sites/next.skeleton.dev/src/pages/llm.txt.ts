 ...
 * (This mimics the logic in your Apitable.astro component.)
 */
...
export const GET: APIRoute = async () => {
  // Load the document you want to transform (using your collection and a specific identifier)
  const doc = await getEntry('docs', 'components/accordion/svelte');
  let content = doc.body; // The raw MDX/astro content

  // ── STEP 1: Build import mappings ─────────────────────────────
  // For preview blocks: find imports that end with "?raw"
  ...
  // For API table schema imports: capture all import statements that do NOT include "?raw"
  const allImportsRegex = /import\s+(\w+)\s+from\s+['"]([^'"]+)['"];/g;
  const schemaImports: Record<string, string> = {};
  while ((match = allImportsRegex.exec(content)) !== null) {
    const identifier = match[1];
    const importPath = match[2];
    if (!importPath.includes('?raw')) {
      schemaImports[identifier] = importPath;
    }
  }

  // ── STEP 2: Replace Preview blocks ─────────────────────────────
  ...
  for (const previewMatch of previewBlocks) {
    ...
    // Replace the preview block with a Markdown code block.
...
\`\`\``;
  ...
  // ── STEP 3: Replace API Table blocks ───────────────────────────
