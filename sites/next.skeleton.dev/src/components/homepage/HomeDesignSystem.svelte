...
<div class="grid grid-cols-1 xl:grid-cols-[0.75fr_2fr] gap-10">
    ...
    <nav class="grid grid-cols-2 gap-4">
      {#each steps as step, i}
        <button
          type="button"
          onclick={() => setState(i)}
          class={`${setStateClass(i)} ${i === 0 ? "col-span-2" : ""}`}
        >
  ...
  <!-- Carousel -->
  <div
    class="card bg-noise preset-filled-primary-500 aspect-video flex justify-center items-center overflow-hidden shadow-xl"
  >
    {#if state === 0}
      <!-- Step 1: Figma -->
      <img
        src="https://i.imgur.com/KOrl47B.png"
        alt="figma"
        class="w-full h-auto"
      />
    {:else if state == 1}
  ...
  "properties": {
    ...
    {:else if state == 2}
      ...
      <div class="grid grid-cols-3 gap-5 md:gap-10">
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-secondary-500"
        ></div>
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-tertiary-500"
        ></div>
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-success-500"
        ></div>
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-error-500"
        ></div>
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-warning-500"
        ></div>
        <div
          class="w-10 md:w-32 aspect-square rounded-full shadow-xl bg-surface-500"
        ></div>
      </div>
    {:else if state == 3}
      <!-- Step 4: Presets -->
      <div class="md:scale-125 xl:scale-150 grid grid-cols-3 gap-4 xl:gap-10">
        ...
        <div class="flex flex-col items-center">
          <button type="button" class="btn preset-tonal hover:preset-filled"
            >Tonal</button
          >
        </div>
        <div class="flex flex-col items-center">
        ...
        <div class="flex flex-col items-center">
          <button type="button" class="btn preset-filled shadow-xl"
            >Elevated</button
          >
        </div>
        <div class="flex flex-col items-center">
    ...
    {:else if state == 4}
      ...
      <div class="text-center space-y-4">
        <h1
          class="heading-font-family text-xs1 md:text-xs3 leading-none font-bold"
        >
          Aa
        </h1>
