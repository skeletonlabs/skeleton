...
export const Preview: React.FC<any> = (props) => {
	...
	const cTab =
		'font-bold py-2 px-4 border-b-[3px] border-transparent capitalize';
	...
	return (
		<div className="space-y-4">
			...
			{/* Panel: Preview */}
			<div
				className={`docs-card-outlined-centered p-8 ${selected !== 'preview' && '!hidden'}`}
			>
				{props.preview}
			</div>
			{/* Panel: Codeblock */}
			<div
				className={`w-full max-w-full ${selected === 'code' ? 'block' : 'hidden'}`}
			>
				{props.code}
			</div>
		</div>
