...
import {
	frameworks,
	isFramework,
	preferredFrameworkStore,
} from "src/stores/preferred-framework";
...
const searchPromise = $derived.by(() => {
	...
	return untrack(async () => {
		...
		const results = await Promise.all(
			result.results.map((result) => result.data()),
		);
  ...
  <article class="[&_mark]:code [&_mark]:text-inherit">
    {#await searchPromise}
      <p class="py-10 text-center">
        <IconLoader class="ml-2 inline size-4 animate-spin" />
      </p>
    {:then results}
        ...
        <ol class="flex flex-col gap-4 space-y-4">
          {#each results as result}
            <li class="space-y-2">
              ...
              <!-- Inner Result -->
              <div
                class="border-surface-200-800 divide-surface-100-900 space-y-2 divide-y-[1px] border-l pl-4"
              >
