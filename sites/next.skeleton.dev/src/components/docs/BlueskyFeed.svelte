...
{:then posts}
  {#each posts as post (post.cid)}
    ...
    >
      <article
        class="card preset-outlined-surface-200-800 hover:preset-tonal p-4 xl:p-10 space-y-4"
      >
        <header class="flex justify-between items-center">
          ...
          <small class="opacity-60"
            >{new Date(post.record.createdAt).toLocaleString()}</small
          >
        </header>
          ...
          {#if post.embed}
            {#if post.embed.$type.includes("external")}
              <div class="block">
                ...
                {:else}
                  <div
                    class="card preset-outlined-surface-200-800 p-4 text-surface-800-200"
                  >
