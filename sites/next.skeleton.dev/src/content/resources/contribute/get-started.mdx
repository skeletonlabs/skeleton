...
> NOTE: non-trivial PRs submitted with our prior consent will be denied. Repeat offenders will be banned.
...
4. Point your terminal at once of the sites or package projects listed below.
5. Run `pnpm i` to install the requirement depedencies.
...
Skeleton uses two primary branches. All pull requests should created from and to the `dev` branch.
...
| `master` | Represents the production branch of the all projects.  | <X size={16} className='inline' /> Never        |

### PR Branch Conventions

Please use the following name convention when creating your pull request.
...
[Changesets](https://github.com/changesets/changesets) are used to automatically generate the Skeleton changelog per each release. Any contributions made to projects that live within the `/packages` directory must contain a Changeset. Use the following instruction to generate a changeset.
...
| `minor`     | For notable changes, such a new features. |
...
We recommend the following official extensions for VS Code:
