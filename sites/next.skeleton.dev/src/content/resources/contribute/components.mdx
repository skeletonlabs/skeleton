...
Note that you may run both the documentation site and each framework package apps in parallel. The following represents the _default_ localhost address and port for each project. This will be displayed in the terminal when starting each dev server.
...
Features can be documented inline within the code via [JSDoc](https://jsdoc.app/about-getting-started). This provides additional context through Intellisense features of text editors and IDEs. These comments are required for all Typescript type definitions. Keep the description short and semantic.
