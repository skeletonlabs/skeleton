...
Tiles are universal between Rails and Bars. They default to buttons, but will convert to anchors when an `href` is provided. You may set the active state with the `selected` property.
