...
Skeleton themes also include contrast color values for every shade. Use these to provide accessible overlapping text and icon fill color styles. Defined via each respective [theme property](/docs/get-started/core-api#colors).
...
These utlities provide a condense utility class syntax that provides an opinioanted pairing of Skeleton's color shades, used to balance your interface elements in light and dark mode. These are enabled throught the use of the CSS's native [light-dark](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark) property.

```
{property}-{color}-{lightModeShade}-{darkModeShade}
```

For example, the `text-primary-300-700` pairing will enable Tailwind to generate the following class:
...
Below is a static representation of each pairing. Only the primary color is shown, but all Skeleton colors are supported.

<ExamplePairings />

Toggle this website bewteen light and dark mode to see how each color reacts.
...
Below is example of setting background colors for each available pairing.
