...
Skeleton themes also include contrast color values for each and every shade. Use these to provide accessible overlapping text and icon fill color styles. Defined via each respective [theme property](/docs/get-started/plugin#colors).
...
Below is a static representation of each pairing.

<ExamplePairings />

Toggle light and dark mode to preview how each pairing automatically adjusts.

<ExamplePairingsDynamic />

Color Parings can be used to generate a heierarchy of visual layers, ranging from foreground to background elements. This uses the same color ramp but inverts the order from light to dark mode.
