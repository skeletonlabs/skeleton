---
layout: '@layouts/LayoutDoc.astro'
title: Presets
description: Canned styles for your interface elements.
srcCore: '/plugin/utilities/presets.css.ts'
order: 20
---

export const components = componentSet;

import Example from '@examples/design/presets/Example.svelte';
import ExampleRaw from '@examples/design/presets/Example.svelte?raw';
import ExampleFilled from '@examples/design/presets/ExampleFilled.astro';
import ExampleFilledRaw from '@examples/design/presets/ExampleFilled.astro?raw';
import ExampleTonal from '@examples/design/presets/ExampleTonal.astro';
import ExampleTonalRaw from '@examples/design/presets/ExampleTonal.astro?raw';
import ExampleOutlined from '@examples/design/presets/ExampleOutlined.astro';
import ExampleOutlinedRaw from '@examples/design/presets/ExampleOutlined.astro?raw';
import ExampleCustom from '@examples/design/presets/ExampleCustom.astro';
import ExampleCustomRaw from '@examples/design/presets/ExampleCustom.astro?raw';
import ExampleGradient from '@examples/design/presets/ExampleGradient.astro';
import ExampleGradientRaw from '@examples/design/presets/ExampleGradient.astro?raw';

{/* prettier-ignore */}
<p class="text-xl"> Presets are complex utility classes that combine various Skeleton primitives. Allowing you quickly style buttons, badges, cards, and more. Create any combination of styles by mixing Skeleton Presets and Tailwind primitives.</p>

<Preview client:load>
	<Fragment slot="preview">
		<Example client:load />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleRaw} lang="svelte" />
	</Fragment>
</Preview>

1. **Filled** - a filled preset of the primary brand color.
2. **Tonal** - a tonal preset of the primary brand color.
3. **Outlined** - an outlined preset of the primary brand color.
4. **Elevated** - mixes a filled preset with a shadow.
5. **Ghost** - has no style by default, but shows a tonal preset on hover.
6. **Ghost Icon** - has no style by default, but shows a branded tonal preset on hover.
7. **Gradient** - a custom preset generated using Tailwind gradient primitives.

## Enabling Presets

Presets are optional, but we recommend users start with our built-ins enabled by default, including: filled, tonal, and outlined. Presets are enabled by importing the following in your global stylesheet.

```css
@import '@skeletonlabs/skeleton/optional/presets';
```

## Built-Ins

### Filled

```
preset-filled-{color}-{lightModeShade}-{darkModeShade}
```

<Preview client:load>
	<Fragment slot="preview">
		<ExampleFilled />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleFilledRaw} lang="astro" />
	</Fragment>
</Preview>

### Tonal

```
preset-tonal-{color}
```

<Preview client:load>
	<Fragment slot="preview">
		<ExampleTonal />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleTonalRaw} lang="astro" />
	</Fragment>
</Preview>

### Outlined

```
preset-outlined-{color}-{shade}-{shade}
```

<Preview client:load>
	<Fragment slot="preview">
		<ExampleOutlined />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleOutlinedRaw} lang="astro" />
	</Fragment>
</Preview>

## Custom Presets

### Create a Preset

Presets are pre-generated combination of Skeleton and Tailwind primitives, which means you can abstract these to create your own set. Add these classes to your global stylesheet and use anywhere you would use the built-in presets. Use the `preset-{foo}` naming scheme to keep things standardized.

<Preview client:load>
	<Fragment slot="preview">
		<ExampleCustom />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleCustomRaw} lang="astro" />
	</Fragment>
</Preview>

### Gradient Presets

Tailwind provides a number of powerful [Gradient](https://tailwindcss.com/docs/gradient-color-stops) utility classes. Combine these to create your own unique gradient presets.

<Preview client:load>
	<Fragment slot="preview">
		<ExampleGradient />
	</Fragment>
	<Fragment slot="code">
		<Code code={ExampleGradientRaw} lang="astro" />
	</Fragment>
</Preview>
