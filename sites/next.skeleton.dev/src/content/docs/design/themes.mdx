---
layout: '@layouts/LayoutDoc.astro'
title: Themes
description: The Skeleton theme system.
order: 0
---

import ThemesList from '@components/docs/ThemeList.astro';

export const components = componentSet;

{/* prettier-ignore */}
<p class="type-scale-5"> Themes are the heart of the Skeleton's design system, empowered by [CSS custom properties](https://developer.mozilla.org/en-US/docs/Web/CSS/--*), authored as CSS-in-JS to enable simple integration into Skeleton's Tailwind plugin. Which allows for simple pre-registration and switching on demand.</p>

---

## Preset Themes

Skeleton comes with a selection of high quality preset themes out of the box. Tap one of the theme preview cards below to copy the theme name to your clipboard.

<ThemesList />

Add the following import to the top of your `tailwind.config`, then continue to the register instructions below.

```ts title="tailwind.config"
import * as themes from '@skeletonlabs/skeleton/themes';
```

## Custom Themes

Optionally you can choose to generate a custom theme to adapt to your app's unique design aesthetic. Skeleton provides a powerful but easy to use theme generator to make to enable this.

<figure class="card-filled-enhanced flex justify-center p-8">
	<a href="https://themes.skeleton.dev/" target="_blank" class="btn preset-filled">
		Theme Generator &rarr;
	</a>
</figure>

1. Open the Theme Generator and design your theme.
2. Tap the "code" view from the menu options at the top of the site.
3. Provide a unique name for your theme.
4. Copy the contents of the generated theme in full.
5. Create a new file in the root of your project called `theme-custom.ts` (any name is fine).

You may import each custom theme at the top of your project's `tailwind.config`. Then proceed to the register instructions below.

```ts title="tailwind.config"
import myCustomTheme from './theme-custom.ts';
```

## Register Themes

Register each imported theme within the Skeleton plugin settings in `tailwind.config`. Then proceed to activation below.

```ts title="tailwind.config" {6-8, 10}
plugins: [
	// The Skeleton Tailwind Plugin
	skeleton({
		themes: [
			// Preset Theme(s)
			themes.cerberus,
			themes.pine,
			themes.rose,
			// Custom Theme(s)
			myCustomTheme
		]
	})
];
```

> TIP: There's no limited to how many themes you can register, but each increases the size of your generated CSS bundle.

## Activate a Theme

Finally, set the active theme to display using the `data-theme` attribute on your `<body>` element.

```html
<body data-theme="cerberus">
	...
</body>
```

---

## Customize and Extend

### Theme Properties

You may update the CSS Custom Properties provided by Skeleton's preset themes. Use the following syntax and apply these styles changes in your global stylesheet. This will target the root scope, a specific theme, then set each property value.

{/* prettier-ignore */}
```css title="app.css"
:root [data-theme='cerberus'] {
    --base-font-family: system-ui;
    --heading-font-family: system-ui;
}
```

### Target Themes

If your application supports multiple themes, you can target changes to specific themes using the `data-theme` attribute.

```css title="app.css"
/** Cerberus theme h1 tags only. */
[data-theme='cerberus'] h1 {
    background: pink;
}
/** Mono theme h1 tags only. */
[data-theme='mona'] h1 {
    background: green;
}
```

### Backgrounds

Your application background color is set automatically using two specific theme properties. These can be overwritten in your global stylesheet using the following [theme properties](/docs/get-started/plugin#colors).

- Light Mode: `—body-background-color`
- Dark Mode: `—body-background-color-dark`

You can also adjust the values directly by targetting the `<body>` element.

```css title="app.css"
/* Pink in light mode, green in dark mode. */
body { @apply bg-pink-500 dark:bg-green-500; }
```

Background images are also supported, including CSS mesh gradients. By using theme properties for each color, you can ensure the gradient adheres to your theme's palette. We recommend Mesher to generate your own custom mesh gradients.

<figure class="card-filled-enhanced flex justify-center p-8">
	<a
		class="btn preset-filled"
		href="https://csshero.org/mesher/"
		target="_blank"
	>
		Mesher by CSS Hero &rarr;
	</a>
</figure>

```css title="app.css"
[data-theme='cerberus'] {
	background-image:
		radial-gradient(at 20% 10%, rgba(var(--color-secondary-500) / 0.075) 0px, transparent 40%),
		radial-gradient(at 75% 10%, rgba(var(--color-primary-500) / 0.075) 0px, transparent 50%);
	background-attachment: fixed;
	background-position: center;
	background-repeat: no-repeat;
	background-size: cover;
}
```

See how we handle [targetted theme gradients](https://github.com/skeletonlabs/skeleton/blob/next/sites/next.skeleton.dev/src/styles/app.css#L44) for this website.


### Custom Fonts

Skeleton recommends the use of [Fontsource](https://fontsource.org/) for custom fonts.


<figure class="card-filled-enhanced flex justify-center p-8">
	<a
		class="btn preset-filled"
		href="https://fontsource.org/"
		target="_blank"
	>
		Browse Fontsource &rarr;
	</a>
</figure>

Install your font of choice.

```console
npm install @fontsource/open-sans
```

You can then import each font at the top of your global stylesheet.

```css title="app.css"
import "@fontsource/open-sans";
```

Then overwrite each applicable [theme property](/docs/get-started/plugin#base-1) for your target theme.

```css title="app.css"
:root [data-theme='cerberus'] {
    --heading-font-family: "Open Sans", sans-serif;
    /* --base-font-family: ... */
    /* --anchor-font-family: ... */
}
```

For custom themes, these settings are can be defined directly in each respective theme file.

---

## API Reference

For more information, please refer to the full [Tailwind Plugin documentation](/docs/get-started/plugin).
