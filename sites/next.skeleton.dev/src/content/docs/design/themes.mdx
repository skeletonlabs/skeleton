...
You may update the CSS Custom Properties provided by Skeleton's preset themes. Use the following syntax and apply these styles changes in your global stylesheet. This will target the root scope, a specific theme, then set each property value.
...
If your application supports multiple themes, you can target changes only to specific themes using the following technique.
...
Your default application background color is set automatically using two specific theme properties. These can be overwritten in your global stylesheet using the Theme Properties technique described above.

- Light Mode: `—body-background-color`
- Dark Mode: `—body-background-color-dark`

You can also adjust the values directly by targetting the `<body>` element.
...
Background images are also supported, including CSS mesh gradients. By setting each color with theme properties, you can ensure the gradient adheres to your theme's unique palette. We recommend the use of Mesher to generate your own custom mesh gradients.
...
See how we handle [targetted theme gradients](https://github.com/skeletonlabs/skeleton/blob/next/sites/next.skeleton.dev/src/styles/app.css#L44) for this website.


### Custom Fonts

Skeleton recommends the use of [Fontsource](https://fontsource.org/) for custom fonts.
...
You can then import each font at the top of your global stylesheet.

```css title="app.css"
import "@fontsource/open-sans";
```

Then overwrite each applicable [theme property](/docs/get-started/plugin#base-1) for your target theme.
...
For custom themes, these settings are can be defined directly in each respective theme file.
