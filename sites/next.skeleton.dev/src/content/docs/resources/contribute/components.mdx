...
| --------------------------- | ----------------------------- | ------------------------------------ |
| `/packages/skeleton-svelte` | [Svelte](https://svelte.dev/) | [SvelteKit](https://kit.svelte.dev/) |
...
- **Zag** - when applicable, these are rest props that are spread directly into the Zag machine.
...
> TIP: Use this convention in React as well, but without `$derived` as React already rerenders when stateful props change.
...
<Control bind:value name="flavors">
...
return (
	<Control value={value} onValueChange={setValue} name="flavors">
		<Control.Item id="chocolate" value="chocolate">
			Chocolate
		</Control.Item>
		<Control.Item id="strawberry" value="strawberry">
			Strawberry
		</Control.Item>
	</Control>
);
