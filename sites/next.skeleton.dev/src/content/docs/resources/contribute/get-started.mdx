...
> NOTE: Make sure you're running PNPM version `9.5.0` or higher. This is because Skeleton makes use of [PNPM catalogs](https://pnpm.io/catalogs) which where introduced in `9.5.0`. You can check your version with `pnpm --version`.
...
| `/packages/skeleton-cli`    | The Skeleton CLI, used for migrations, etc.                               |
| `/packages/skeleton-common` | The Skeleton common package, containing shared internal only utilities.   |
...
Run the following commands in the monorepo root:
