...
<p class="type-scale-5">Skeleton supports a variety of web-based frameworks and meta-frameworks, so we've created the following guide to serve as a universal reference when implementing page layouts. These techniques utilizes native HTML and Tailwind, meaning Skeleton is supported but not required.</p>
...
5. May introduce inconsisent behavior between web framework layout systems.
...
Let's start by creating the several traditional layouts using a combination of semantic HTML and Tailwind utility classes.
...
> TIP: Use [backdrop-blur](https://tailwindcss.com/docs/backdrop-blur) produce a hazy glass-like effect for overlapped semi-transparent elements.
