---
layout: '@layouts/LayoutDoc.astro'
title: Buttons
description: Provide a variety of button, including customizable sizes and types.
srcCore: '/plugin/components/buttons.css'
showDocsUrl: true
---

export const components = componentSet

import Example from "@examples/tailwind/buttons/Example.astro";
import ExampleRaw from "@examples/tailwind/buttons/Example.astro?raw";
import ExamplePresets from "@examples/tailwind/buttons/ExamplePresets.astro";
import ExamplePresetsRaw from "@examples/tailwind/buttons/ExamplePresets.astro?raw";
import ExampleSizes from "@examples/tailwind/buttons/ExampleSizes.astro";
import ExampleSizesRaw from "@examples/tailwind/buttons/ExampleSizes.astro?raw";
import ExampleDisabled from "@examples/tailwind/buttons/ExampleDisabled.astro";
import ExampleDisabledRaw from "@examples/tailwind/buttons/ExampleDisabled.astro?raw";
import ExampleGroup from "@examples/tailwind/buttons/ExampleGroup.astro";
import ExampleGroupRaw from "@examples/tailwind/buttons/ExampleGroup.astro?raw";

<Preview client:load>
    <Fragment slot="preview"><Example /></Fragment>
    <Fragment slot="code"><Code code={ExampleRaw} lang="astro" /></Fragment>
</Preview>

## Presets

<Preview client:load>
    <Fragment slot="preview"><ExamplePresets /></Fragment>
    <Fragment slot="code"><Code code={ExamplePresetsRaw} lang="astro" /></Fragment>
</Preview>

## Sizes

<Preview client:load>
    <Fragment slot="preview"><ExampleSizes /></Fragment>
    <Fragment slot="code"><Code code={ExampleSizesRaw} lang="astro" /></Fragment>
</Preview>

## Disabled

<Preview client:load>
    <Fragment slot="preview"><ExampleDisabled /></Fragment>
    <Fragment slot="code"><Code code={ExampleDisabledRaw} lang="astro" /></Fragment>
</Preview>

## Group

<Preview client:load>
    <Fragment slot="preview"><ExampleGroup /></Fragment>
    <Fragment slot="code"><Code code={ExampleGroupRaw} lang="astro" /></Fragment>
</Preview>

