---
layout: '@layouts/LayoutDoc.astro'
title: Badges
description: Provides a robust set of non-interactive badge styles.
srcCore: '/plugin/components/badges.css'
showDocsUrl: true
---

export const components = componentSet

import Example from "@examples/tailwind/badges/Example.astro";
import ExampleRaw from "@examples/tailwind/badges/Example.astro?raw";
import ExamplePresets from "@examples/tailwind/badges/ExamplePresets.astro";
import ExamplePresetsRaw from "@examples/tailwind/badges/ExamplePresets.astro?raw";
import ExampleOverlap from "@examples/tailwind/badges/ExampleOverlap.astro";
import ExampleOverlapRaw from "@examples/tailwind/badges/ExampleOverlap.astro?raw";

<Preview client:load>
    <Fragment slot="preview"><Example /></Fragment>
    <Fragment slot="code"><Code code={ExampleRaw} lang="astro" /></Fragment>
</Preview>

## Presets

<Preview client:load>
    <Fragment slot="preview"><ExamplePresets /></Fragment>
    <Fragment slot="code"><Code code={ExamplePresetsRaw} lang="html" /></Fragment>
</Preview>

## Overlap

<Preview client:load>
    <Fragment slot="preview"><ExampleOverlap /></Fragment>
    <Fragment slot="code"><Code code={ExampleOverlapRaw} lang="astro" /></Fragment>
</Preview>